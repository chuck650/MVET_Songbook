...
const SONGS_DIR     = path.join(WORKSPACE_DIR, 'public/songs');
const OUTPUT_FILE   = path.join(WORKSPACE_DIR, 'public/songs.json');
...
function getMusicXMLKey(fifths, mode) {
  const keys = {
    'major': {
      ...
      '-1': 'F major', '-2': 'B♭ major', '-3': 'E♭ major', '-4': 'A♭ major', '-5': 'D♭ major', '-6': 'G♭ major', '-7': 'C♭ major'
    },
    'minor': {
      '0': 'A minor', '1': 'E minor', '2': 'B minor', '3': 'F♯ minor', '4': 'C♯ minor', '5': 'G♯ minor', '6': 'D♯ minor', '7': 'A♯ minor',
      '-1': 'D minor', '-2': 'G minor', '-3': 'C minor', '-4': 'F minor', '-5': 'B♭ minor', '-6': 'E♭ minor', '-7': 'A♭ minor'
    }
  };
...
function generateManifest() {
  ...
  for (const folder of fs.readdirSync(SONGS_DIR)) {
    ...
    const mxlPath = mainFiles.mxl ? path.join(WORKSPACE_DIR, 'public', mainFiles.mxl) : null;
    ...
    const msczPath = mainFiles.mscz ? path.join(WORKSPACE_DIR, 'public', mainFiles.mscz) : null;
    ...
    // Normalise key capitalisation and enforce standard music notation Unicode accidentals
    if (meta.key) {
      meta.key = meta.key
        .replace(/Major/gi, 'major')
        .replace(/Minor/gi, 'minor')
        .replace(/\b([A-G])b\b/g, '$1♭')
        .replace(/\b([A-G])b(?=\s)/g, '$1♭')
        .replace(/\b([A-G])#\b/g, '$1♯')
        .replace(/\b([A-G])#(?=\s)/g, '$1♯');
    }
